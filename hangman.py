...
def get_num_attempts():
    #get user input for no of incorrect attempts
    while True:
        num_attempts = input(
...
def get_min_word_length():
   #get minimum word length input from user
    while True:
...
def get_next_letter(remaining_letters):
    #get user letter input/guess
    if len(remaining_letters) == 0:
        raise ValueError('There are no remaining letters')
    while True:
        next_letter = input('Enter the next letter: ').lower()
...
def play_hangman():
    #play game 
	#let user return if wants to retry
    ...
    while attempts_remaining > 0 and not word_solved:
        ...
        if next_letter in word:
            # Guessed correctly
            print('{0} is in the word! :O'.format(next_letter))
            ...
            # Guessed incorrectly
            print('{0} is NOT in the word! :c'.format(next_letter))
